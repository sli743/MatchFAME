...
function P_est = MinimumSpanningTree(SMat,dim,d,m,AdjMat,X)
t_max = 100;
...
while sum(added)<sum(C==1)
    for node_root = rootnodes
        ...
        for node_leaf=leaves
            ...
            matrix2permutation(X((blk_ind(node_leaf)+1):blk_ind(node_leaf+1), (blk_ind(node_root)+1):blk_ind(node_root+1)) * ...
