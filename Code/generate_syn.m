...
function[XMat, XMat_gt, mat_size, AdjMat, GoodEdge] = generate_syn(n,n_pt,p,q0,q1,q2)

    % n=100;
    % n_pt = 10000;
    % p=0.1;  % prob of connection
    % 
    % 
    % 
    % %%%%%%%%% two probabilities of corruption:
    % q0=0.01;
    % q1=0.1; % keypoint match corruption (elementwise)
    % q2=0; % partial perm corruption (the entire matrix)
    sigma=0.0; % noise for generating ground truth camera orientations; sigma=0 means cameras exactly point at the origin
    G = rand(n,n) < p;
    G = tril(G,-1);
    disp('generate initial adjacency matrix')
    AdjMat = G + G'; 
    ...
    disp('generate camera locations from 3d gaussian dist')
    T_gt = 10*randn(3,n);
    ...
    for k=1:m
        ...
        if num_common>=5  %%%%%%%% criterion for connecting two images
           AdjMat(i,j)=1;
           AdjMat(j,i)=1;
        end
    end
    %No isolated image, for now
    for i=1:n
        if sum(AdjMat(i,:))==0
            AdjMat(i,mod(i,n)+1)=1;
            AdjMat(mod(i,n)+1,i)=1;
        end
    end
    GoodEdge = sparse(n,n);
    G = tril(AdjMat,-1);
    ...
    %Essential_gt = cell(1,m);
    %Fundamental_gt = cell(1,m);
    ...
    for k=1:m
        ...
        %tij = T_gt(:,i)-T_gt(:,j);
        %Tij = ssc(tij);
        %Essential_gt{k} = R_gt(:,:,i)*Tij*(R_gt(:,:,j))';     % ground truth essential mat
        %Fundamental_gt{k} = (inv(K))'*Essential_gt{k}*inv(K); % ground truth fundamental mat
        ...
        if temp<q2     % with prob q2, completely corrupt the partial perm by randomly permute rows/columns of the matrix
           ...
           GoodEdge(j,i) = -1;
            ...
            for l = zero_row'   % if there is no match for the keypoint
                if n_col>0
                    temp = rand;
                    if temp<=q1   % with prob q1 add a random matrch
    ...
    %GT contains all matches
    for i=1:n
        for j=1:n
            R = Abs_perm_gt{i}*(Abs_perm_gt{j})';     
            XMat_gt( Xind(i)+1: Xind(i+1),  Xind(j)+1: Xind(j+1))=R;
            XMat_gt( Xind(j)+1: Xind(j+1),  Xind(i)+1: Xind(i+1))=R';
        end
    end
    1-sum(XMat_gt.*XMat,'all')/sum(XMat_gt,'all');
end


%(sum(XMat_gt,'all')+sum(XMat,'all')-2*sum(XMat_gt.*XMat,'all'))/(sum(XMat_gt,'all')+sum(XMat,'all')-sum(XMat_gt.*XMat,'all'))
